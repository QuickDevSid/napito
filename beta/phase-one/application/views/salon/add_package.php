												...
												if(!empty($package_list)){
                                                    $i=1;
                                                        foreach($package_list as $package_list_result){                                                             
                                                            ...
                                                            $products = explode(',',$package_list_result->product_name);
                                                            $product_details_str = '';
                                                            if (empty($products)) {
                                                                $product_details_str = '-';
                                                            } else {
                                    ...
                                    <div class="col-md-12 col-sm-5 col-xs-12">
                                        <table id="example1" class="table table-striped responsive-utilities jambo_table" style="width:100%;">
                                            ...
                                            <tbody>
                                                ...
                                                <?php if(!empty($ready_packages)){
                                                    // echo "<pre>";print_r($ready_packages);exit;
                                                    ...
                                                    $i=1;
                                                        foreach($ready_packages as $ready_packages_result){ 
                                                            ...
                                                            // echo "<pre>";print_r($final_price);exit;
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?=$ready_packages_result->reward_point?></td>
                                                    <td><?=$ready_packages_result->count_value?> <?=$ready_packages_result->count_type?></td>
                                                    <td class="status-column-hidden">
                                ...
                                <form method="post" class="form-group" name="customer_form" id="customer_form" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            ...
                                            <input autocomplete="off"  type="text" class="form-control" name="package_name" id="package_name" value="<?php if (!empty($single_setup_package)) {echo $single_setup_package->package_name;} ?>" placeholder="Enter package name">
											 <input autocomplete="off" type="hidden" name="id" id="id" value="<?php if (!empty($single_setup_package) && isset($_GET['edit'])) { echo $single_setup_package->id; } ?>">
                                        ...
                                        <div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            ...
                                            <input autocomplete="off"  type="text" class="form-control" name="package_name_marathi" id="package_name_marathi" value="<?php if (!empty($single_setup_package)) {echo $single_setup_package->package_name_marathi;} ?>" placeholder="Enter package name in मराठी">
                                        </div>
                                        <div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            <label> Gender<b class="require">*</b></label>
                                            <input type="hidden" class="form-control" name="gender" id="gender" value="<?php if (!empty($single_setup_package)) { echo $single_setup_package->gender;} ?>">
                                            <input type="text" readonly class="form-control" name="gender_name" id="gender_name" value="<?php if(!empty($single_setup_package)){ if($single_setup_package->gender=="0"){ echo "Male";}else{ echo "Female";}}?>">
                                        </div>
                                        </div>
                                    <div class="row">

                                        <div class="col-md-8 col-sm-6 col-xs-12 form-group service_name">
										
											   <?php 
													$actual_price = 0;
													$service_exp = [];
													if(!empty($single_setup_package)){
														$service_exp = explode(",",$single_setup_package->service_name);
                                                        $service_title = $this->Salon_model->get_services_for_offers_genderwise($single_setup_package->gender);
													} 
                                                    // echo '<pre>'; print_r($service_title);
												?>
                                    ...
                                    <!-- <div class="row"> 
                                        <div class="col-md-8 col-sm-6 col-xs-12 form-group product_name_validation">
                                            <label>Products<b class="require">*</b></label>
                                            <select style="border: solid gray 1px;" class="form-select form-control chosen-select" name="product_name[]" id="product_name" multiple>  
                                                <?php 
                                                    if (!empty($single_setup_package)) {
                                                        $selected_products = $this->Salon_model->get_product_for_package_by_service_during_update_new($single_setup_package->service_name);

                                                        $displayed_products = [];

                                                        if (!empty($selected_products)) {
                                                            foreach ($selected_products as $selected_products_res) {
                                                                echo '<option disabled>' . $selected_products_res->service_name . ' Products - ' . '</option>';
                                                                
                                                                if (!empty($selected_products_res->products)) {
                                                                    foreach ($selected_products_res->products as $data) {
                                                                       
                                                                        if (!in_array($data->product_name, $displayed_products)) {
                                                                            echo '<option value="' . $data->id . '" disabled selected>' . $data->product_name . '</option>';
                                                                         
                                                                            $displayed_products[] = $data->product_name;
                                                                        }
                                                                    }
                                                                } else {
                                                                    echo '<option disabled>Products not available</option>';
                                                                }
                                        ...
                                        <div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            ...
                                            <input type="hidden" name="old_package_image" id="old_package_image" value="<?php if(!empty($single_setup_package)){ echo $single_setup_package->package_image; }?>">
                                        </div>
                                        ...
                                        <div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            ...
                                            <input autocomplete="off" readonly type="text" class="form-control" name="actual_price" id="actual_price" value="<?php if(!empty($single_setup_package)){ echo $single_setup_package->actual_price;} ?>" placeholder="Actual price">
                                        </div>
										...
										<div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            ...
                                            <input autocomplete="off"  type="text" onkeyup="set_discount()" class="form-control" name="discount" id="discount" value="<?php if(!empty($single_setup_package)) { echo $single_setup_package->discount; } ?>" placeholder="Enter discount">
                                            <div class="discount_validation error"></div>
                                        </div>
										<?php 
											$final_price = 0;
											if(!empty($single_setup_package) && $single_setup_package->discount_in == 0){
												$part = $single_setup_package->discount;
												$total = $single_setup_package->actual_price; 
                                                if($total != 0 && $total != "" && $total != "0"){
												    // $percentage = ($part / $total) * 100;
                                                    $percentage = $total -($total * ($part/100));
												...
												// $final_price = $single_setup_package->actual_price-$percentage;
												$final_price = $percentage;

											}else if(!empty($single_setup_offer)){
												$final_price = $single_setup_package->actual_price-$single_setup_package->discount;
                                                // echo "<pre>";print_r($final_price);exit;
                                            }
										?>
                                        <div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            <label>Final Price <b class="require">*</b></label>
                                            <input autocomplete="off"  readonly type="text" class="form-control" name="amount" id="amount" value="<?php if(!empty($single_setup_package)){ echo $single_setup_package->amount;} ?>" placeholder="Final price">
                                        </div>
                                        <div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            ...
                                            <select class="form-select form-control col-md-4 col-xs-12" name="count_type" id="count_type">
                                                ...
                                                <option value="Days" <?php if (!empty($single_setup_package) && $single_setup_package->count_type == "Days") echo 'selected="selected"'; ?>>Days</option>
                                                <option value="Week" <?php if (!empty($single_setup_package) && $single_setup_package->count_type == "Week") echo 'selected="selected"'; ?>>Week</option>
                                                <option value="Month" <?php if (!empty($single_setup_package) && $single_setup_package->count_type == "Month") echo 'selected="selected"'; ?>>Month</option>
                                                <option value="Year" <?php if (!empty($single_setup_package) && $single_setup_package->count_type == "Year") echo 'selected="selected"'; ?>>Year</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="row">
										<div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                            ...
                                            <input autocomplete="off"  type="number" class="form-control" name="count_value" id="count_value" value="<?php if (!empty($single_setup_package)) { echo $single_setup_package->count_value; } ?>" placeholder="Enter package expiry number">
                                        </div>
									
                                        <div class="col-md-4 col-sm-6 col-xs-12 form-group">
                                                <label>Reward Point <b class="require">*</b></label>
                                                <input autocomplete="off"  type="text" class="form-control" name="reward_point" id="reward_point" value="<?php if (!empty($single_setup_package)) {echo $single_setup_package->reward_point; } ?>" placeholder="Enter reward point">
                                            </div> 
                                            <div class="form-group col-md-4">
                                                <div class="custom_fields_data">
                                                    <label for="department_master">Button Background Color <b class="require">*</b></label>
                                                    <input autocomplete="off"  type="text" class="color_value_input" name="bg_color_input" pattern="^#+([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$" value="<?php if(!empty($single_setup_package)){ echo $single_setup_package->bg_color;}else{ echo '#bada55';} ?>" id="hexcolor"></input>
                                                    <input autocomplete="off"  type="color" id="colorpicker" class="color_input" name="bg_color" pattern="^#+([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$" value="<?php if(!empty($single_setup_package)){ echo $single_setup_package->bg_color;}else{ echo '#bada55';} ?>"> 
                                                </div>
                                            </div>
										</div>
                                    <div class="row">
                                        <div class="form-group col-md-4">
                                                <div class="custom_fields_data">
                                                    ...
                                                    <input autocomplete="off"  type="text" class="button_text_color_input" name="text_color_input" pattern="^#+([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$" value="<?php if(!empty($single_setup_package)){ echo $single_setup_package->text_color;}else{ echo '#000000';} ?>" id="texthexcolor"></input>
                                                    <input autocomplete="off"  type="color" id="textcolorpicker" class="text_color_input" name="text_color" pattern="^#+([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$" value="<?php if(!empty($single_setup_package)){ echo $single_setup_package->text_color;}else{ echo '#000000';} ?>"> 
                                                </div>
                                            </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-8 col-sm-6 col-xs-12 form-group">
                                            ...
                                            <textarea class="form-control" name="description" id="description" placeholder="Enter description"><?php if (!empty($single_setup_package)) {echo $single_setup_package->description; } ?></textarea>
                                        </div> 
                                    </div>
                                    <pre></pre>
                                    <div class="row">
                                        ...
                                        <!-- <div class="col-md-8 col-sm-6 col-xs-12 form-group" style="text-align:right;">
                                            <label class="error" style="font-size:10px;">(Note: Products are automatically added for the selected services.)</label>
                                        </div> -->
                                    </div>
                                </form> 
                            </div> 
   ...
   $("#service_name").change(function() {
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#product_ids').empty();
            $("#product_name").empty();
            // $('#product_name').append('<option value="">Select Products</option>');
            $.each(parsedData, function(i, d) {
                tttt = tttt + parseFloat(d.final_price); 
                $('#product_name').append('<option disabled>' + d.service_name + ' Products -</option>');
                
                if(d.products.length === 0) {
                    $('#product_name').append('<option disabled>No products available</option>');
                } else {
                    $.each(d.products, function(j, product) {
                        tttt = tttt + parseFloat(product.selling_price); 
                        $('#product_name').append('<option selected disabled value="' + product.id + '">' + product.product_name + '</option>');
                        $('#product_ids').append(product.id);
                    });
                }
                
                $('#product_name').append('<option disabled>--------------------------------</option>');
            });
