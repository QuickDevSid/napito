...
class Admin_controller extends CI_Controller {
	...
	public function admin_dashboard(){
		$data['low_coin_balance_branch'] = $this->Admin_model->get_low_coin_balance_branch();
		$data['branch_renewals'] = $this->Admin_model->get_branch_subscription_renewals();
			// echo '<pre>'; print_r($data['branch_renewals']); exit;
	...
	public function add_salon(){
		...
		if($this->form_validation->run() === FALSE){
			...
			if(!empty($data['single'])){
				if($data['single']->is_branch_available == '0'){
					$data['self_branch'] = $this->Admin_model->get_branch_details($data['single']->self_branch_id);
				}
			}
			$data['salons'] = $this->Admin_model->get_all_salon_except($this->uri->segment(2));
			...
			$pancard_copy = "";
			if($_FILES['pancard_copy']['name'] !=""){
				$temp = explode('.', $_FILES['pancard_copy']['name']);
				$ext = end($temp);
				$pancard_copy = $temp[0]."_".round(microtime(true)) . '.' . end($temp);
				$config = array(
					'upload_path' 	=> "admin_assets/images/owner_aadhar/",
					'allowed_types' => "pdf|jpg|jpeg|png",
					'file_name'	=> $pancard_copy,
				);			
				$this->upload->initialize($config);
				if($this->upload->do_upload('pancard_copy')){
					$data = $this->upload->data();				
					$pancard_copy = $data['file_name'];	
				}else{ 
					$error = array('error' => $this->upload->display_errors());	
					$this->upload->display_errors();
				}
			}else{
				$pancard_copy = $this->input->post('old_pancard_copy');
			} 
			$shopact = "";
	...
	public function branch_payment(){
		...
		if($this->form_validation->run() === FALSE){
			...
			redirect('branch_payment_history?branch='.($this->input->post('branch') != "" ? $this->input->post('branch') : $this->input->post('selected_branch')));
		}
	} 
	...
	public function branch_wp_coin_history(){
		$data['branch_details'] = $this->Admin_model->get_branch_details(isset($_GET['branch']) ? $_GET['branch'] : '');
		// echo '<pre>'; print_r($data['branch_details']); exit;
		$this->load->view('admin/branch_wp_coin_history',$data);
	}
	...
	public function add_branch(){
		$this->form_validation->set_rules('branch_name_0','Branch Name','required'); 
		if($this->form_validation->run() === FALSE){
			$data['setup'] = $this->Master_model->get_backend_setups();
			...
			$result=$this->Admin_model->set_bulk_branch();
			if($result){
				$this->session->set_flashdata('success','Branches added successfully. Please make the payment entries to activate the subscriptions');
			}else{
				$this->session->set_flashdata('message','Something went wrong, Please try again');
			}
			redirect('add-branch/' . $this->uri->segment(2));
		}
	} 
	public function update_branch(){
		...
		if($this->form_validation->run() === FALSE){
			...
			$data['state'] = $this->Admin_model->get_state_list();
			$data['salon_type'] = $this->Admin_model->get_type_of_salon_with_rules();
			$this->load->view('admin/update_branch',$data);
		}else{
			$shopact = "";
			if($_FILES['shopact']['name'] !=""){
				$temp = explode('.', $_FILES['shopact']['name']);
	...
	public function add_subscription(){
		...
		if($this->form_validation->run() === FALSE){
			...
			$data['wp_feature_list'] = $this->Admin_model->get_all_subscription_wp_feature(51);
	...
	public function add_sub_category(){
		...
		if($this->form_validation->run() === FALSE){
			...
			$result = $this->Admin_model->add_sub_category($category_image);
			if($result == "0"){
				$this->session->set_flashdata('success','Record added successfully');
			}else{
				$this->session->set_flashdata('success','Record updated successfully');
	...
	public function add_admin_service(){
		if(isset($_GET['pending'])){
			...
			if($this->form_validation->run() === FALSE){
				$data['single'] = $this->Admin_model->get_single_salon_service();
				$this->load->view('admin/update_pending_service',$data);
			}else{
				$category_image = "";
				if($_FILES['category_image']['name'] !=""){
					$temp = explode('.', $_FILES['category_image']['name']);
				...
				$result = $this->Admin_model->update_pending_service($category_image);
	...
	public function admin_product_add(){
		if(isset($_GET['pending'])){
			...
			if($this->form_validation->run() === FALSE){
				...
				$data['single_product'] = $this->Salon_model->get_single_product_id($this->uri->segment(2)); 
				$this->load->view('admin/update_pending_product',$data);
			}else{
				$product_photo = array();
				...
				$this->Admin_model->update_pending_product($product_photo);
				$this->session->set_flashdata('success','Record updated successfully');
	...
	public function addon_requests(){
		$this->load->view('admin/addon_requests');
	} 
	...
	public function inactivate_booking_rule_setup(){
		$success = $this->Admin_model->inactivate_booking_rule_setup();
		if ($success) {
			$this->session->set_flashdata('success', 'Record inactivated successfully');
		}else{
			$this->session->set_flashdata('error', 'Failed to inactivate the record');
	...
	public function activate_booking_rule_setup(){
		$success = $this->Admin_model->activate_booking_rule_setup();
		if ($success) {
			$this->session->set_flashdata('success', 'Record activated successfully');
		}else{
			$this->session->set_flashdata('error', 'Failed to activate the record');
	...
	public function all_website_contact_list(){
		$data['website_contact'] = $this->Admin_model->get_website_contact_list();
		$this->load->view('admin/website_contact_list',$data);
	}

	

	public function all_business_contact_list(){
		$data['business_contact'] = $this->Admin_model->get_business_contact_list();
		$this->load->view('admin/business_contact_list',$data);
	}

	public function whatsapp_addon_plans(){
		$this->form_validation->set_rules('subscription_id','Subscription','required'); 
		if($this->form_validation->run() === FALSE){
			$data['subscription'] = $this->Admin_model->get_single_subscription();
			$data['whatsapp_plans'] = $this->Admin_model->get_subscription_whatsapp_addon_plans($this->uri->segment(2));
			$data['single'] = $this->Admin_model->get_single_subscription_whatsapp_addon_plan($this->uri->segment(3),$this->uri->segment(2));
			$this->load->view('admin/whatsapp_addon_plans',$data);
		}else{			
			if($this->input->post('submit') == 'insert_form'){
				$result = $this->Admin_model->set_whatsapp_addon_plans();
				if ($result == "0") {
					$this->session->set_flashdata('message', 'Something went wrong');
				} else {
					$this->session->set_flashdata('success', 'Record added successfully');
				...
				$result = $this->Admin_model->update_whatsapp_addon_plans();
				if ($result == "0") {
					$this->session->set_flashdata('message', 'Something went wrong');
				} else {
					$this->session->set_flashdata('success', 'Record updated successfully');
			...
			redirect('whatsapp-addon-plans/' . $this->uri->segment(2));
		}
	} 
}   