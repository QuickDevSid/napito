                            ...
                            <form method="post" name="master_form" id="master_form" enctype="multipart/form-data">
                                <div class="row">
                                    <div class="form-group col-lg-4 col-md-4 col-sm-6 col-xs-12">
                                        <label>Select Status Name <b class="require">*</b></label>
                                            <select class="form-select form-control" name="status_name" id="status_name">
                                                <option value="">Select Status Name</option>
                                                <?php if (!empty($color_list)) {
                                                    foreach ($color_list as $category_result) { ?> 
                                                        <option value="<?= $category_result->id ?>" <?php if (!empty($single) && $single->status_name == $category_result->id) { ?>selected="selected" <?php } ?>><?=$category_result->status_name?></option>
                                                    <?php }
                                                } ?>
                                        </select>
                        ...
                        <table id="example" class="table table-striped">
                            ...
                            <tbody>
                                <?php if (!empty($status_list)) {
                                    $i = 1;
                                    foreach ($status_list as $color_list_result) {
                                ?>
                                        <tr>
                                            <td scope="row">
                                                <?= $i++ ?>
                                            ...
                                            <td><button style="cursor: default;height: 25px;border-radius: 5px;color: white;border: 1px solid #ccc;background-color: <?= $color_list_result->status_color ?>;"><?= $color_list_result->status_name ?></button></td>
                                            <td>
                                                ...
                                                <a title="Edit" class="btn btn-success" href="<?= base_url() ?>set-booking-status/<?=$color_list_result->id ?>"><i class="fa-solid fa-pen-to-square"></i></a>
                                            </td>
                                        </tr>
                                <?php }
                                } ?>
...
<script>
    $(document).ready(function() {
        $('#master_form').validate({
            ...
            messages: {
                status_name: "Please select status name",
