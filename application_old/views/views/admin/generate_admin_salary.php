					...
					<div class="container">
						<form name="add_student" id="add_student" autocomplete="off" method="POST"
										...
										<select id="staff" name="staff" class="form-control info_input">
											...
											<?php if (!empty($employee)) {
												foreach ($employee as $employee_result) { ?>
													<option value="<?= $employee_result->id; ?>">
														<?= $employee_result->full_name; ?>
													</option>
												<?php }
											} ?>
										</select>
							...
							<div class="row"> -->
								...
								<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12" style="margin-top:4px">
									<div class="form-group">
										<label>Remark</label>
							...
							<div class="row"> -->
								...
								<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
									<div class="form-group deduct_amt_input" style="display:none;">
							...
							<div class="row">
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
									<button style="margin-top: 15px;" class="form_submit btn btn-success">Submit
									</button>
								</div>
							</div>
						</form>
...
<script type="text/javascript">
	...
	$('.info_input').change(function () {
		...
		if (staff_id != "" && from_date != "" && to_date != "") {
			$.ajax({
				type: "POST",
				url: "<?= base_url(); ?>admin/Ajax_controller/get_already_generated_field_exe_slip_ajx",
	...
	function field_exe_attendance() {
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url(); ?>admin/Ajax_controller/get_employee_present_days_ajx",
			data: { 'staff_id': staff_id, 'from_date': from_date, 'to_date': to_date },
			success: function (data) {
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url(); ?>admin/Ajax_controller/get_employee_half_days_ajx",
			data: { 'staff_id': staff_id, 'from_date': from_date, 'to_date': to_date },
			success: function (data) {
				$('#half_days').val(data);
			},
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url(); ?>admin/Ajax_controller/get_employee_absent_days_ajx",
			data: { 'staff_id': staff_id, 'from_date': from_date, 'to_date': to_date },
			success: function (data) {
	...
	$('#staff').change(function () {
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url(); ?>admin/Ajax_controller/get_employee_total_loan_ajx",
