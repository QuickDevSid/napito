							...
							<form name="add_salary" id="add_salary" autocomplete="off" method="POST"
								...
								<div class="row">
									<div class="form-group col-md-4 col-sm-6 col-xs-12">
										...
										<select id="staff" name="staff" class="form-control info_input paid_date" <?=!empty($single) ? 'disabled' : ''; ?>>
											<option value="">Select Employee</option>
											<?php if (!empty($employee)) {
                                                foreach ($employee as $employee_result) { ?>
                                                    <option value="<?= $employee_result->id ?>" <?=!empty($single) && $single->emp_id == $employee_result->id ? 'selected' : ''; ?>><?= $employee_result->full_name?> <?=$employee_result->designation_name != "" ? ' [' . $employee_result->designation_name . ']' : ''; ?></option>
                                                <?php }
                                            } ?>
										</select>
									</div>
									<div class="form-group col-md-4 col-sm-6 col-xs-12">
										...
										<input type="text" readonly name="from_date" id="from_date"
											class="form-control paid_date" placeholder="DD-MM-YYYY" value="<?=!empty($single) && $single->from_date != "" ? date('d-m-Y',strtotime($single->from_date)) : ''; ?>">
									</div>
									<div class="form-group col-md-4 col-sm-6 col-xs-12">
										<label>To Date<b class="require">*</b></label>
										<input type="text" readonly name="to_date" id="to_date"
											class="form-control paid_date" placeholder="DD-MM-YYYY" value="<?=!empty($single) && $single->to_date != "" ? date('d-m-Y',strtotime($single->to_date)) : ''; ?>">
									</div>
									<input type="hidden" name="id" id="id" value="<?=!empty($single) ? $single->id : ''; ?>">
									<div class="form-group col-md-4 col-sm-6 col-xs-12">
										<label>Days</label>
								...
								<div class="row">
									<div class="form-group col-md-12 col-sm-12 col-xs-12">
										...
										<textarea name="reason" id="reason"
											class="form-control" placeholder="Enter Reason"><?=!empty($single) ? $single->reason : ''; ?></textarea>
									</div>
								</div>
...
<script type="text/javascript">
	...
	function validateLeavePeriod(staff_id,from_date,to_date){
		...
		$.ajax({
            ...
            data: { 'staff_id': staff_id, 'from_date': foramted_from_date, 'to_date': foramted_to_date, 'id': $('#id').val() },
            success: function (data) {
				var opts = $.parseJSON(data);
				if(opts.leave_period_flag == '2'){
					$('#submit_leave').attr('disabled',true);
					$('#validate-error').show();
					$('#validate-error').html('Attendance already marked for one of the date in selected date range');
				}else{
					if(opts.leave_period_flag == '1'){
						if(opts.active_bookings_flag == '1'){
							$('#submit_leave').attr('disabled',true);
							$('#validate-error').show();
							$('#validate-error').html('Bookings available for the selected leave period, Please transfer the bookings first. <a style="text-decoration:underline;" href="<?php echo base_url();?>transfer-bookings?stylist='+ staff_id + '">Transfer Now</a>');
						}else{
