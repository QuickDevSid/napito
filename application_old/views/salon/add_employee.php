                          ...
                          <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="text-align:right;">
                            ...
                            <div class="row" style="<?php if (empty($upcoming_appointments)) { echo 'display:block;'; } else { echo 'display:none;'; } ?>"> 
                              <div class="form-group col-md-8 col-sm-6 col-xs-12 service_name_validation" id="services_selection_div">
                                <!-- Male Section -->
                                <?php $service_name = $this->Salon_model->get_salon_services_list_for_emp('0'); ?>
                                <div class="form-group col-md-6 col-sm-6 col-xs-6" style="<?=!empty($service_name) ? '' : 'display:none;'; ?>">
                                  <label>Male Services <b class="require">*</b></label>
                                  <label style="float:right;">Check All 
                                    <input type="checkbox" class="check-all" data-gender="male" id="check_all_male">
                                  </label>
                                  <div class="checkbox-container">
                                    <?php 
                                    if (!empty($service_name)) {
                                      $service_exp = !empty($single) ? explode(",", $single->service_name) : [];
                                      foreach ($service_name as $single_service_name) {
                                    ?>
                                      <label class="service-name">
                                        <input type="checkbox" class="services-checkbox male" name="service_name[]" value="<?= $single_service_name->id ?>" <?= in_array($single_service_name->id, $service_exp) ? 'checked' : '' ?>>
                                        <small><?= $single_service_name->subcategory . ' | ' . $single_service_name->sub_category_marathi . ' -> '; ?></small>
                                        <?= $single_service_name->service_name . ' | ' . $single_service_name->service_name_marathi; ?>
                                      </label>
                                    <?php }}else{ ?>
                                      <label class="service-name"><small>Services not available.</small></label>
                                    <?php } ?>
                                  </div>
                                </div>

                                <!-- Female Section -->
                                <?php $service_name = $this->Salon_model->get_salon_services_list_for_emp('1'); ?>
                                <div class="form-group col-md-6 col-sm-6 col-xs-6" style="<?=!empty($service_name) ? '' : 'display:none;'; ?>">
                                  <label>Female Services <b class="require">*</b></label>
                                  <label style="float:right;">Check All 
                                    <input type="checkbox" class="check-all" data-gender="female" id="check_all_female">
                                  </label>
                                  <div class="checkbox-container">
                                    <?php 
                                    if (!empty($service_name)) {
                                    ...
                                    ?>
                                      <label class="service-name">
                                        <input type="checkbox" class="services-checkbox female" name="service_name[]" value="<?= $single_service_name->id ?>" <?= in_array($single_service_name->id, $service_exp) ? 'checked' : '' ?>>
                                        <small><?= $single_service_name->subcategory . ' | ' . $single_service_name->sub_category_marathi . ' -> '; ?></small>
                                        <?= $single_service_name->service_name . ' | ' . $single_service_name->service_name_marathi; ?>
                                      </label>
                                    <?php }}else{ ?>
...
$(document).ready(function() {
      ...
      // Function to update "Check All" when individual boxes are toggled
      $('.services-checkbox').on('change', function() {
        updateCheckAllState('male');
        updateCheckAllState('female');
      });

      // On page load: Check if all respective services are selected
      function updateCheckAllState(gender) {
        let total = $('.' + gender).length;
        let checked = $('.' + gender + ':checked').length;
        $('#check_all_' + gender).prop('checked', total > 0 && total === checked);
      }
