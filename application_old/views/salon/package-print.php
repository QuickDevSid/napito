...
<?php if(!empty($customer_amount)){  
		...
		// echo '<pre>'; print_r($single_profile); exit;
  $services = explode(',',$customer_amount->all_service_name);
  $services_str = '';
  if (empty($services)) {
      $services_str = '-';
  } else {
      for ($k = 0; $k < count($services); $k++) {
          $service_details = $this->Salon_model->get_service_details_all($services[$k],$customer_amount->branch_id,$customer_amount->salon_id);
          if (!empty($service_details)) {
            if ($service_details->service_name != "" && $service_details->service_name_marathi != "") {
              $single_html = $service_details->service_name . "|" . $service_details->service_name_marathi . ', ';
    ...
    <div class="tm_invoice_wrap">
      <div class="tm_invoice tm_style1 tm_type1" id="tm_download_section">
        <div class="tm_invoice_in">
          ...
          <div class="tm_card_note tm_mobile_hide"><b class="tm_primary_color">Payment Status: </b><?php if($customer_amount->is_booking_done == '1'){ echo 'Completed'; }else{ echo 'Pending'; } ?></div>
          <div class="tm_invoice_info_list tm_white_color">
              ...
              <p class="tm_invoice_date tm_m0" >Date: <b><?php echo ($customer_amount->payment_on != "") ? date('d/m/Y',strtotime($customer_amount->payment_on)) : '-'; ?></b></p>
            </div>
            ...
            <div class="tm_invoice_left">
			 <p class="tm_mb2"><b class="tm_primary_color">Receipt To:</b></p>
              <p>
                ...
                Package: <?=$customer_amount->package_name?>
                ...
                <table>
                  <thead>
                    <tr class="tm_accent_bg">
                      <th class="tm_width_4 tm_semi_bold tm_white_color" style="width:15%;">Description</th>
                      <th class="tm_width_4 tm_semi_bold tm_white_color" style="width:20%;">Package</th>
                      <th class="tm_width_2 tm_semi_bold tm_white_color" style="width:40%;">Services</th>
                      <th class="tm_width_2 tm_semi_bold tm_white_color" style="width:20%;">Period</th>
                      <th class="tm_width_1 tm_semi_bold tm_white_color" style="width:5%;">Amount</th>
                    </tr>
                  </thead>
                  <?php $sold_by = $this->Salon_model->get_single_emp_details($customer_amount->allocated_by); ?>
                  <tbody>
                    <tr>
						<td class="tm_width_4" style="width:15%;"><b>Package Payment</b></td>
						<td class="tm_width_2" style="width:20%;"><b><?=$customer_amount->package_name?></b></td>
						<td class="tm_width_2" style="width:40%;"><b><?=rtrim($services_str, ', ')?></b></td>
						<td class="tm_width_2" style="width:20%;"><b><?= ($customer_amount->package_start_date != "" && $customer_amount->package_end_date != "") ? date('d M Y',strtotime($customer_amount->package_start_date)).' to '.date('d M Y',strtotime($customer_amount->package_end_date)) : '-';?></b></td>
						<td class="tm_width_2 tm_text_right" style="width:5%;"><b><?=number_format((float)$customer_amount->package_amount, 2, '.', '')?></b></td>
                    </tr> 
                  </tbody>
                </table>
              </div> 
                ...
                <table class="tm_mb15">
                  ...
                  <?php if ($customer_amount->discount_in_rs > 0) { ?>
                    <tr class="tm_gray_bg "  style="background: #0080002e;border-top: 1px solid #ccc;">
                      ...
                      <td class="tm_width_3 tm_primary_color tm_bold tm_text_right"><?=$customer_amount->discount_in_rs?></td>
                    </tr>
                    <?php } ?>
                    <!-- <tr class="tm_gray_bg ">
                      <td class="tm_width_3 tm_primary_color tm_bold">Subtoal</td>
                      <td class="tm_width_3 tm_primary_color tm_bold tm_text_right"><?=number_format(((float)$customer_amount->package_amount - ($customer_amount->discount_in_rs != "" ? $customer_amount->discount_in_rs : 0.00)), 2, '.', '')?></td>
                    </tr> -->
                    <?php if($customer_amount->is_gst_applicable == '1'){ ?>
                             <tr class="tm_gray_bg">
                        ...
                        <?php } ?>
                            <tr class="tm_accent_bg">
                                ...
                                <td class="tm_width_3 tm_border_top_0 tm_bold tm_f16 tm_white_color tm_text_right"><?php echo number_format(((float)($customer_amount->package_amount - ($customer_amount->discount_in_rs != "" ? $customer_amount->discount_in_rs : 0.00)) + $customer_amount->gst_amount), 2, '.', ''); ?></td>
                            </tr>
