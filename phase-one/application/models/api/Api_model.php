...
class Api_model extends CI_Model {
        ...
        if($request){
            if($request['mobile_number']){
                ...
                if($exist > 0){
                    ...
                    $number = $mobilenumber;
                    $customer = '';
                    // $template_id = '1107173349090818309';
                    $template_id = '1107173493417554781';
    ...
    public function get_store_profile(){
        ...
        if($request){
            if($request['branch_id']){
                ...
                $this->db->select('tbl_store_profile.*,tbl_branch.salon_address as branch_salon_address,tbl_branch.pincode as branch_salon_pincode');
                ...
                if(!empty($profile)){                                     
                    ...
                    $data['phone_number'] = $profile->salon_number;
        ...
        if($request){
            if($request['branch_id']){
                ...
                if(!empty($single)){ 
                    ...
                    $this->db->order_by('tbl_admin_service_category.order', 'asc');
                        ...
                        foreach($services as $services_result){
                            ...
                            if($services_result->discount_type == '1'){    //Flexible
                                ...
                                if($services_result->discount_in == '0'){  //percentage
                                    ...
                                    $offer_text = $services_result->service_discount.'% Off';
                                }elseif($services_result->discount_in == '1'){ //flat
                                    $discount_amount = (float)$services_result->service_discount;
                                    $offer_text = 'Flat Rs. '.$services_result->service_discount.' Off';
                                }
                            }
                            ...
                            $offer_text .= $services_result->reward_point != ""  && $services_result->reward_point != "0" ? ' & ' . $services_result->reward_point.' Reward Point' : '';
                            ...
                            if(!$is_emergency){
                                ...
                                if($working_hrs['is_allowed'] == 1){
                                    ...
                                    if (empty($morning_slots) && empty($afternoon_slots) && empty($evening_slots)) {
                                        $json_arr['status'] = 'false';
                                        $json_arr['message'] = 'No time slots available';
                                    } else {
                                        $data = array(
                ...
                if(!empty($profile)){
                    ...
                    if(!empty($single)){ 
                        ...
                        if($booking_date != "" && $slot_from != ""){    
                            // $this->db->where('service_start_date',date("Y-m-d",strtotime($booking_date)));
                            // $this->db->where('service_start_time',date("H:i:s",strtotime($slot_from)));
                            // $this->db->where('is_deleted','0');
                            // $overlapping_booking = $this->db->get('tbl_new_booking')->num_rows();
                            $overlapping_booking = $this->Salon_model->get_overlapping_bookings($services,$branch_id,$salon_id);
                            ...
                            if($overlapping_booking == 0){
                                    ...
                                    for($i=0;$i<count($services);$i++){
                                        ...
                                        if(!empty($single_service)){
                                            ...
                                            if($service_added_from == '0'){
                                                ...
                                                $min_slab = $single_service->min;
                                                $max_slab = $single_service->max;
                                                $slab_increment = $single_service->slab_increment;
                                                $slab_consider = '';
                                                $discount_amount = 0;
                                                if($single_service->discount_type == '1'){    //Flexible
                                                    $customer_last_service_booking = $this->Salon_model->get_customer_last_service_booking($customer_id,$single_service->id);
                                                    if(!empty($customer_last_service_booking)){                                        
                                                        $min_slab = $single_service->min;
                                                        $max_slab = $single_service->max;
                                                        $slab_increment = $single_service->slab_increment;
                                                    ...
                                                    if($single_service->discount_in == '0'){  //percentage
                                                        $discount_amount = ((float)$slab_consider * (float)$single_service->final_price) / 100;
                                                    }elseif($single_service->discount_in == '1'){ //flat
                                                        $discount_amount = (float)$slab_consider;
                                                    }
                                                }elseif($single_service->discount_type == '0'){   //Fixed
                                                    if($single_service->discount_in == '0'){  //percentage
                                                        $discount_amount = ((float)$single_service->service_discount * (float)$single_service->final_price) / 100;
                                                    }elseif($single_service->discount_in == '1'){ //flat
                                                        $discount_amount = (float)$single_service->service_discount;
                                                    }
                                                }
                                ...
                                if(!empty($customer_details) && $customer_details->customer_phone != "" && $customer_details->customer_phone != null && $customer_details->customer_phone != '0000000000'){
                                    ...
                                    if(!empty($booking_details)){
                                        ...
                                        if(count($services) > 0){
                                            ...
                                            $this->Salon_model->send_notification($app_message,$title,$notification_data,$message,$number,$type,$customer,$salon_id,$branch_id,$for_order_id,$for_offer_id,$generated_from,$for_query_id,$message_send_on,$template_id,$email_subject,$email_html,$consent_form_id);
                                        }
                                    }
                                }
                                ...
                                //             $type = 'sender';
                                //             $project = 'salon';
                                //             $message = 'Received booking for ' . $emp_details->full_name . ' on ' . date('d M, Y h:i A', strtotime($booking_details->booking_date.' '.$booking_details->service_start_time));
                                //             $data = json_encode([
                                ...
                                //                 'service_from' => $booking_details->booking_date.' '.$booking_details->service_start_time,
                                //                 'service_to'   => $booking_details->booking_date.' '.$booking_details->service_end_time,
                                //                 'service_date' => date('Y-m-d', strtotime($booking_details->booking_date)),
                                //                 'service_name' => !empty($service_details) ? $service_details->service_name : '',
                                //                 'stylist_name' => $emp_details->full_name
                                //             ]);
                                //             $socket_response = $this->Salon_model->send_data_to_socket_client('app',$uid,$type,$project,$message,$data);
                                //         }
                                //     }
                                // }
                                ...
                                $json_arr['status'] = 'false';
                                $json_arr['message'] = 'Booking date and Timeslot is not available';
                                $json_arr['data'] = [];
                            }
                        }else{
                            $json_arr['status'] = 'false';
                                ...
                                for($i=0;$i<count($reschedule_details);$i++){
                                    ...
                                    if(!empty($single_booking_services)){ 
                                        ...
                                        $update_data = array(
                                            ...
                                            'service_from'          =>  date('Y-m-d H:i:s',strtotime($reschedule_details[$i]['service_from'])),
                                            'service_to'            =>  date('Y-m-d H:i:s',strtotime($reschedule_details[$i]['service_to']))
                                        );
                                        ...
                                        $reschedule_data = array(
                                            ...
                                            'new_details'       =>  date('Y-m-d H:i:s',strtotime($reschedule_details[$i]['service_from'])).'@@@'.date('Y-m-d H:i:s',strtotime($reschedule_details[$i]['service_to'])).'@@@'.$reschedule_details[$i]['stylist_id'],
                                            'created_on'        =>  date("Y-m-d H:i:s")
                                        );
                        ...
                        if(!empty($services)){
                            foreach($services as $services_result){
                                if(in_array($services_result->id,$services_to_cancel)){
                                    ...
                                    //     if($services_result->service_reward_points != "" && $services_result->service_reward_points != null){
                                    //         $deduct_rewards = $services_result->service_reward_points;
                                    //     }else{
                                    //         $deduct_rewards = 0;
                                    //     }
                                    //     $this->db->where('branch_id',$branch_id);
                ...
                if(!empty($profile)){
                    ...
                    if(!empty($single)){ 
                        ...
                        if($payment_status == '1'){                        
                            ...
                            $single_giftcard = $this->db->get('tbl_gift_card')->row();
                            if(!empty($single_giftcard)){
                                $gift_card_balance = $single_giftcard->gift_price;
                                if($gift_card_balance > 0){
                                    $gift_card_status = '0';
                                }elseif($gift_card_balance == 0){
                                    $gift_card_status = '1';
                            ...
                            for($i = 0; $i < $service_count; $i++){                               
                                ...
                                if(!empty($single_service)){
                                    ...
                                    $services_array[] = array(
                                        ...
                                        'category_marathi_name'     =>  $single_service->sup_category_marathi,
                                        'sub_category_marathi_name' =>  $single_service->sub_category_marathi,
                                        'products'          =>  $products,
                ...
                if(!empty($single)){ 
                    ...
                    if(!empty($packages)){ 
                        ...
                        foreach($packages as $active_package_allocation){       
                            $this->db->where_in('id',explode(',',$active_package_allocation->package_service_name));
                            $this->db->where('branch_id', $branch_id);
                            $this->db->where('salon_id', $salon_id);
    ...
    public function get_customer_booking($api_type,$customer_id,$salon_id,$branch_id,$booking_id,$booking_status,$service_status,$limit,$offset){
        $this->db->select('tbl_new_booking.*,tbl_salon_customer.full_name as customer_full_name,tbl_salon_customer.customer_phone');
            ...
            foreach($bookings as $bookings_result){  
                ...
                if(!empty($booking_service_details)){ 
                    foreach($booking_service_details as $booking_service_details_result){ 
                        $services_text .= rtrim($booking_service_details_result->service_name,' ').', ';
                        ...
                        $services_data[] = array(
                            ...
                            'service_from'          =>  date('h:i A',strtotime($booking_service_details_result->service_from)),
                            'service_to'            =>  date('h:i A',strtotime($booking_service_details_result->service_to)),
                ...
                if ($bookings_result->payment_status == '0' && ($bookings_result->booking_status === '1' || $bookings_result->booking_status === '3' || $bookings_result->booking_status === '4')) {
                    ...
                    if ($allowed_rescheduling_time > $currentDateTime) {
                        $is_reschedule_allowed = '1';
                    }
                    if ($allowed_cancel_time > $currentDateTime) {
                        $is_cancellation_allowed = '1';
                    }
                }
                ...
                $data[] = array(
                    ...
                    'from'              =>  date('h:i A',strtotime($bookings_result->service_start_time)),
                    'to'                =>  date('h:i A',strtotime($bookings_result->service_end_time)),
                ...
                if(!empty($single)){ 
                    ...
                    if(!empty($memberships)){ 
                        ...
                        foreach($memberships as $memberships_result){                             
                            ...
                            if($discount_type == '0'){
                                $service_discount_text = $memberships_result->service_discount.'%';
                                $product_discount_text = $memberships_result->product_discount.'%';
                            }elseif($discount_type == '1'){
                                $service_discount_text = 'Flat Rs. ' . $memberships_result->service_discount;
                                $product_discount_text = 'Flat Rs. ' . $memberships_result->product_discount;
                            }else{
                            ...
                            $data[] = array(
                                ...
                                'discount_text'     =>  $service_discount_text . ' Discount on Services and ' . $product_discount_text . ' Discount On Products',
        ...
        if($request){
            if(isset($request['store_code'])){
                ...
                if(!empty($branch)){
                    ...
                    $this->db->where('salon_id',$branch->salon_id);
                    $branch_profile = $this->db->get('tbl_store_profile')->row();
                    if(!empty($branch_profile)){                    
                        $this->db->select('tbl_store_reviews.*,tbl_salon_customer.full_name,tbl_salon_customer.profile_pic');
                        $this->db->join('tbl_salon_customer','tbl_salon_customer.id = tbl_store_reviews.customer_id');
    ...
    public function set_store(){
        ...
        // if (!$this->authenticate($auth_token)) {
        //     $json_arr = [
        //         'status' => 'false',
