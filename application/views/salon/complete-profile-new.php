...
<div class="complete_profile_container">
    ...
    <div class="intro-container">
        ...
        <?php
                ...
                if(in_array($next_status_flag,$allowed_status)){
        ?>
            <div>
                <a id="skip_button" class="btn btn-default back_button_class" onclick="return confirm('Are you sure you want to skip <?=$next_status_name; ?>?');" href="<?=base_url();?>skip-onboarding?status=<?=base64_encode($next_status_flag); ?>&redirect=<?=base64_encode($skip_link); ?>&skip_step=<?=base64_encode($next_status_name); ?>">Skip <?=$next_status_name; ?></a>
            </div>
        <?php }else{ ?>
            <div>
                <!-- <a id="skip_button" class="btn btn-default back_button_class" href="<?=base_url();?>store-profile">Skip To Continue</a> -->
                <a id="skip_button" class="btn btn-default back_button_class" onclick="return confirm('Are you sure you want to skip <?=$next_status_name; ?>?');" href="<?=base_url();?>skip-onboarding?status=<?=base64_encode($next_status_flag); ?>&redirect=<?=base64_encode($skip_link); ?>&skip_step=<?=base64_encode($next_status_name); ?>">Skip <?=$next_status_name; ?></a>
            </div>
        <?php }} ?>
    </div>
