    ...
    <div class="">
        <div class="page-title row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <h3>
                    Add New Booking
                    <a class="<?php if(empty(array_intersect(['product-booking'], $feature_slugs))) { echo 'blurred '; }?>btn btn-primary" id="product_booking_anchor" style="float:right;display:none;" href="<?=base_url();?>product-booking">Purchase Products</a>
                </h3>
            </div>
        </div>
                            ...
                            <div class="container">
                                <div class="row">
                                    <div class="col-md-7 col-sm-12 col-xs-12">
                                        <input type="hidden" id="is_late_hr" name="is_late_hr" value="0">
                                        <input type="hidden" id="late_booking_start_time_slot_selected" name="late_booking_start_time_slot_selected" value="">
                                        <input type="hidden" id="late_employee_selected" name="late_employee_selected" value="">
                                ...
                                <!-- <div class="row service-payment-title hhh_ccc_span">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="text-align:center;">
                                        <a class="btn btn-primary" id="late_hr_button"onclick="showLateHrDiv()" data-toggle="modal" data-target="#LateHrModal" >Late Hour Booking</a>
                                    </div>
                                    <div id="selected_late_hr_details"></div>
                                </div> -->
                                <div class="row service-payment-title hhh_ccc_span">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                ...
                                <hr class="break_line"> -->
                                <input type="hidden" id="marketing_service_discount_amount_hidden" name="marketing_service_discount_amount_hidden" value="0.00">
...
<div  class="loader-new"></div>
    ...
    <div class="add-new-customer-main" style="display: none;">
        <div class="add-new-customer-content" >
            <form method="post" name="add_customer_form" id="add_customer_form" enctype="multipart/form-data">
                ...
                <input type="hidden" name="hidden_start" id="hidden_start" value="<?php if(isset($_GET['start'])){ echo $_GET['start']; }?>">
                <input type="hidden" name="hidden_end" id="hidden_end" value="<?php if(isset($_GET['end'])){ echo $_GET['end']; }?>">
                <input type="hidden" name="hidden_stylist" id="hidden_stylist" value="<?php if(isset($_GET['stylist'])){ echo $_GET['stylist']; }?>">

                <input type="hidden" name="added_from" id="added_from" value="booking-form">
    ...
    <div class="modal fade" id="LateHrModal"  tabindex="-1" aria-labelledby="LateHrModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="LateHrModalLabel" style="display: flex; justify-content: space-between; align-items: center;">
                        <span>Late Hour Booking</span>
                    </h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close" style="float:none !important; position:absolute;right:10px;top:10px;" onclick="closePopup('LateHrModal')">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" id="late_hour_response"></div>
            </div>
        </div>
    </div>
...
<script>
                ...
                if(user_selected_service.length == 0 && user_selected_products.length == 0){
                    ...
                    if($('#customer_name').val() == ''){
                        ...
                        if (validation_flag == 1) {
                            ...
                            $("#stylist_timeslot_error").html('Please Ensure stylists are available for all the selected service'); 
                        }
                    }
                }
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (opts.length > 0) {
                        ...
                        // $('#customers').html('Customer Not Found ! <b onclick="open_customer_model(\'guest_to_parmanant\')" style="cursor:pointer;" class="add-new-customer">Add Customer</b>');
                ...
                success: function(data) {     
                    ...
                    if(parsedData.customer.is_guest == '0'){
                        ...
                        if (packages.length > 0) {
                            $('#package_div').append('<a class="customer_packages_available" href="<?= base_url(); ?>bill-generation?customer=' + parsedData.customer.id + '&bill_for=1" style="text-decoration:underline;" target="_blank">Assign New</a>');
                            $('#package_name').html('');
                            ...
                            $('#package_div').append('<div class="customer_packages_not_available">Packages not available. <a href="<?= base_url(); ?>bill-generation?customer=' + parsedData.customer.id + '&bill_for=1" style="text-decoration:underline;" target="_blank">Assign New</a></div>');
                        }
                    }
    ...
    function setServicePrice(serviceID){
        var service_original_price = parseFloat($('#service_original_price_' + serviceID).val());
            ...
            // if (booking_date !== "" && booking_start !== "") {
                if ($('#service_name_check_' + serviceID).is(':checked')) {
                    if(!user_selected_service.includes(serviceID)){
                        ...
                        current_total = current_total + service_original_price;

                        user_selected_service.push(serviceID);

                        createServiceDetailsDiv(serviceID,service_added_from,service_name,service_duration,service_rewards,' (Rs. '+service_original_price+')');
    ...
    function createServiceDetailsDiv(serviceID,selected_from,service_name,service_duration,service_rewards,price){
        ...
        service_details = 
            '<style>#service_stylist_id_' + serviceID + '_chosen{ pointer-events: auto !important; background-color: #e9ecef; color: #6c757d; }</style>' +
    ...
    function getTimeStylist(booking_date,selected_slot_start,selected_slot_end,serviceID,service_duration){
        ...
        if (formatted_start !== "" && typeof formatted_start !== "undefined" && formatted_end !== "" && typeof formatted_end !== "undefined") {
            ...
            $.ajax({
                ...
                data: { 'booking_date':booking_date,'service':serviceID,'selectedTimeSlot': selectedTimeSlot, 'selected_employee': selected_employee, 'employee_selection_rule': employee_selection_rule, 'previous_stylist': previous_stylist, source:'vendor_panel' },
                success: function(data) {
                    ...
                    if(stylists.length > 0){
                        ...
                        $("#service_stylist_id_"+serviceID).append('<option value="">Stylist not available. Select different slot.</option>').trigger('chosen:updated');
                        $("#service_stylist_id_"+serviceID+"_chosen").show();
    ...
    function calculateTotalDiscount(){
        ...
        if(user_selected_service.length > 0){
            for(var i=0;i<user_selected_service.length;i++){
                var service_added_from = $('#service_added_from_' + user_selected_service[i]).val();
                if(service_added_from == 'manual' || service_added_from == 'default'){
                    var service_discount_rewards_type = parseFloat($('#service_discount_rewards_type_' + user_selected_service[i]).val());
                    if(service_discount_rewards_type == '0'){
                        var service_received_discount = parseFloat($('#service_received_discount_' + user_selected_service[i]).val());
                        marketing_service_discount_amount += service_received_discount;
                    }
                }
            }
        }
        $('#marketing_service_discount_amount_hidden').val(parseFloat(marketing_service_discount_amount).toFixed(2));

        var marketing_service_discount_amount = parseFloat($('#marketing_service_discount_amount_hidden').val());
        ...
        total_discount = marketing_service_discount_amount + membership_service_discount_amount + membership_product_discount_amount + coupon_discount_amount + giftcard_discount_amount + reward_discount_amount;
    ...
    function setBookingAmount(){
        ...
        marketing_service_discount_amount = parseFloat($('#marketing_service_discount_amount_hidden').val());
        ...
        booking = payable - coupon_discount - reward_discount - marketing_service_discount_amount;
    ...
    function showLateHrDiv() {
        var customer_name = $('#customer_name').val();
        if(customer_name != ""){
            $('.loader_div').show();   
            $.ajax({
                url: "<?= base_url(); ?>salon/Ajax_controller/get_late_hr_div_ajx",
                method: 'POST',
                data: {
                    'source':'vendor_panel',
                    'sub_source':'late_hr_booking',
                    'booking_date': $('#booking_date').val(),
                    'user_selected_service': user_selected_service
                },
                success: function(response) {
                    $('#late_hour_response').html(response);
                    showPopup('LateHrModal');
                    $('.loader_div').hide();   
                },
                error: function(xhr, status, error) {
    ...
    function saveLateHrStylist(){
        $('#late_booking_start_time_slot_selected').val('');
        $('#late_employee_selected').val('');
        $('#is_late_hr').val('0');
        $('#late_booking_start_time_slot_error').hide().text('');
        var late_booking_start_time_slot = $('input[name="late_booking_start_time_slot"]:checked').val();
        if(late_booking_start_time_slot){
            $('#late_booking_start_time_slot_selected').val(late_booking_start_time_slot);
            $('#late_employee_selected').val($('#late_employee').val());
            $('#is_late_hr').val('1');            
            closePopup('LateHrModal');
        }else{
            $('#late_booking_start_time_slot_error').show().text('Please select timeslot');
        }
    }
</script>