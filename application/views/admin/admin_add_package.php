                                        ...
                                        <select class="form-select form-control chosen-select" name="service_name[]" id="service_name" multiple>
                                            ...
                                            if (!empty($services)) {
                                                foreach ($services as $service_name_result) {
                                                    if ($service_name_result->gender == "0") {
                                                        $service_gender = "(Male)";
                                                    } else {
                                                        $service_gender = "(Female)";
                                                    }
                                            ?>
                                                    <option value="<?= $service_name_result->id ?>" <?php if (!empty($single) && in_array($service_name_result->id, $service_exp)) { ?>selected="selected" <?php } ?>><?=$service_name_result->sub_category_name . '|' . $service_name_result->sub_category_marathi; ?> -> <?=$service_name_result->service_name . '|' . $service_name_result->service_name_marathi; ?> <?=$service_gender;?></option>
                                            <?php }
                                            } ?>
                                        </select>
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $.each(opts, function(i, d) {
                        if(d.gender == '0'){
                            var service_gender = ' (Male)';
                        }else{
                            var service_gender = ' (Female)';
                        }
                        if (old_services.includes(d.id.toString())) {
                            $('#service_name').append('<option selected value="' + d.id + '"> ' + d.sub_category_name + '|' + d.sub_category_marathi + ' -> ' + d.service_name + '|' + d.service_name_marathi + '' + service_gender + '</option>');
                        }else{
                            $('#service_name').append('<option value="' + d.id + '"> ' + d.sub_category_name + '|' + d.sub_category_marathi + ' -> ' + d.service_name + '|' + d.service_name_marathi + '' + service_gender + '</option>');
                        }
                    });
