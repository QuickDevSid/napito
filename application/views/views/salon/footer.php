...
?>
                                ...
                                <div class="footer-step <?= $status_key <= $footer_onboarding_status ? 'footer-completed' : ($next_status_flag == (int)$steps[$status_key]['flag'] ? 'footer-next_step' : '') ?>">
                                    ...
                                    <div class="footer-label" title="<?= $steps[$status_key]['name'] ?>">
                                        <?php if ($link_allowed == '1') { ?>
                                            <a href="<?= $steps[$status_key]['link'] ?>"><?= $steps[$status_key]['name'] ?></a>
                                        <?php } else { ?>
                                            <a><?= $steps[$status_key]['name'] ?></a>
                                        <?php } ?>
                                    </div>
    ...
    if ($this->uri->segment(1) != 'complete-profile') {
        if ($this->uri->segment(1) == 'store-profile') {
            ...
            $show_next_button = !empty($this->Salon_model->get_salon_products()) ? true : false;
            $show_skip_button = false;
            $next_url = 'add-salon-services';
        } elseif ($this->uri->segment(1) == 'add-salon-services' || $this->uri->segment(1) == 'salon-services-list' || $this->uri->segment(1) == 'ready-sub-category' || $this->uri->segment(1) == 'ready-services' || $this->uri->segment(1) == 'product-list' || $this->uri->segment(1) == 'use_ready_product_sub_cat' || $this->uri->segment(1) == 'use_ready_product_cat') {
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#alter_btn').click(function() {
            ...
            $('.stylist-sale-tooltip').toggleClass('tooltip_down')
...
<script>
    ...
    var bellSound_path = '<?php echo base_url() . 'salon_assets/bell-ringing-01c.wav'; ?>';
