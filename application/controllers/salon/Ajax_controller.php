...
public function get_product_detail_for_stock_ajax() {
  ...
  public function get_product_for_package_ajax() {
   ...
   public function get_customer_single_details_ajax() {
	$this->Salon_model->get_customer_single_details_ajax();
   }
   public function get_customer_completed_bookings_ajx() {
	$this->Salon_model->get_customer_completed_bookings_ajx();
   }
   public function get_customer_completed_product_bookings_ajx() {
	$this->Salon_model->get_customer_completed_product_bookings_ajx();
   }
   ...
   public function show_bill_update_popup_ajx() {
	$this->Salon_model->show_bill_update_popup_ajx();
   }
   ...
   public function get_category_sub_categories() {
	$this->Salon_model->get_category_sub_categories();
   }
   ...
   public function get_stylistwise_calender_data() {
	$result = $this->Salon_model->get_stylistwise_calender_data();
  ...
  public function get_services_info_ajax() {
	$this->Salon_model->get_services_info_ajax();
  }
  public function get_products_info_ajax() {
	$this->Salon_model->get_products_info_ajax();
  }
  public function get_giftcard_info_ajax() {
	$this->Salon_model->get_giftcard_info_ajax();
  }
...
public function get_all_bookings_ajax(){
	...
	if(!empty($result)){
		...
		foreach($result as $data){
			...
			}elseif($data->is_direct_billing == '1'){
				$background_color = 'background-color:#fff9e5;';
			}
?>
						...
						<div class="row accordian_row_booking">
							...
							<div class="col-lg-8">	
								<div class="row" >
									...
									<div class="col-lg-8">
										<p style="font-size: 13px;color: #414141;"><b>Booking Date: <?= date('d M, Y',strtotime($data->service_start_date));?> <?= $data->service_start_time != "" ? date('h:i A',strtotime($data->service_start_time)).' to '.date('h:i A',strtotime($data->service_end_time)) : '';?></b></p>
									</div>
								</div>
								<div class="row">
									<div class="col-lg-4">
							...
							<?php 
									echo '<button style="margin-right:0px;background:transparent !important;outline:none; box-shadow:none;" title="Update Payment Details" id="updateBill_' . $data->id . '" onclick="showBillUpdatePopup(' . $data->id . ')" data-toggle="modal" data-target="#updateBillModal" class="btn btn-primary event-action-button"><i style="font-size: 17px;color: black;" class="fas fa-edit"></i></button>';
								} 
					...
					<table id="example" class="table  table-bordered dt-responsive nowrap example list_table"  style="width:100%">
						...
						?>
							<tr title="<?=$package_title_name;?>" style="<?=$background_color;?>">
								...
								<td>
                                    <?php 
                                        if($booking_services_result->service_status == '0'){
                                            ...
                                            echo $booking_services_result->completed_on != "" ? '<br>On: '.date('d-m-Y',strtotime($booking_services_result->completed_on)) : '';
								...
								<td>
									<?= date('d M, Y',strtotime($booking_services_result->service_date));?>
								</td>
								<td><?= $booking_services_result->service_from != "" ? date('h:i A',strtotime($booking_services_result->service_from)).' to '.date('h:i A',strtotime($booking_services_result->service_to)) : '-';?></td>
						...
						<div class="row accordian_row_booking">
							...
							<div class="col-lg-9 col-md-9 col-sm-9 col-xs-9">	
								<div class="row" >
									...
									<div class="col-lg-8">
										<p style="font-size: 13px;color: #414141;"><b>Purchase Date: <?= date('d M, Y',strtotime($data->booking_date));?></b></p>
									</div>
								</div>
								<div class="row">
									<div class="col-lg-4">
								...
								<div class="row">
									<div class="col-lg-4">
										<p style="font-size: 11px;color: #414141;margin: 0px;"><b>Order ID: <?=$data->receipt_no;?></b></p>
									</div>
								</div>
							</div>
							<?php if(!empty($booking_review)){ ?>
					...
					<table id="example" class="table  table-bordered dt-responsive nowrap example list_table"  style="width:100%">
						<thead>
							<tr>
								...
								<th>Purchase Price<br><small>(Before Discount)</small></th>
								<th>Purchase Stylist</th>
...
public function get_saloon_customers_ajx(){
	...
	if(!empty($document)){
		...
		foreach($document as $print){
			...
			$sub_array = array();
			$sub_array[] = '<label style="display:none;">' . $print->is_registered_coins_generated . '</label>' . $offset++;
			...
			if(empty($membership_details)){
				$add_mem_button = '<br><a style="color:black"; href="'.base_url().'bill-generation?customer='.$print->id.'&bill_for=0">Add Membership</a>';
			}
...
public function get_customize_message_ajx(){
	$this->Marketing_model->get_customize_message_ajx();
}
...
public function get_message_content_ajx(){
	$this->Marketing_model->get_message_content_ajx();
}
...
public function get_salon_giftcards_ajx(){
	$this->Salon_model->get_salon_giftcards_ajx();
}
...
public function get_customer_report_data_ajx(){
	...
	if(!empty($document)){
		...
		foreach($document as $print){
			...
			$sub_array[] = $offset++;
			$sub_array[] = $print->full_name;
			$sub_array[] = $print->gender == "0" ? 'Male' : ($print->gender == '1' ? 'Female' : '-');
			$sub_array[] = $print->customer_phone;
			$sub_array[] = $print->email != "" ? $print->email : '-';
			...
			$sub_array[] = '<a class="btn btn-success" title="Edit" href="'.base_url().'add-customer/'.$print->id.'?source=report"><i class="fa-solid fa-pen-to-square"></i></a>';
			$data[] = $sub_array; 
		}
	}
...
public function get_customer_package_redemption_report_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
	...
	$document = $this->Salon_model->get_customer_package_redemption_report_data_ajx_list($length, $start, $search);
	// echo '<pre>'; print_r($document);
	$data = array();
	if(!empty($document)){
		$page = $start / $length + 1;
		...
		foreach($document as $print){
			...
			$sub_array[] = $print->package_name . '<br>Price: Rs. ' . $print->package_amount;
			$sub_array[] = $print->sup_category . '|' . $print->sup_category_marathi . ' -> ' . $print->subcategory . '|' . $print->sub_category_marathi;
			$sub_array[] = $print->service_name . '|' . $print->service_name_marathi . '<br>Price: Rs. ' . $print->service_final_price;
			$sub_array[] = '<a style="text-decoration:underline;" target="_blank" href="'.base_url().'booking-list?id='.$print->booking_id.'">' . $print->receipt_no . '</a>';
			$sub_array[] = $print->stylist_full_name != "" ? $print->stylist_full_name : '-';
			$sub_array[] = date('d-m-Y', strtotime($print->service_date));
			$sub_array[] = $print->is_direct_billing == '1' ? '-' : date('h:i A', strtotime($print->service_from)) . ' to ' . date('h:i A', strtotime($print->service_to));
			$data[] = $sub_array; 
		}
	}
	$TotalProducts = $this->Salon_model->get_customer_package_redemption_report_data_ajx_count($search);
...
public function get_membership_report_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
	...
	$document = $this->Salon_model->get_membership_report_data_ajx_list($length, $start, $search);
	// echo '<pre>'; print_r($document);
	$data = array();
	if(!empty($document)){
		$page = $start / $length + 1;
		$offset = ($page - 1) * $length + 1;
		foreach($document as $print){
			$sold_by = $this->Salon_model->get_single_emp_details($print->employee_id);

			$sub_array = array();
			$sub_array[] = $offset++;
			$sub_array[] = $print->full_name;
			$sub_array[] = $print->customer_phone;
			$sub_array[] = '<button class="btn btn-sm" style="float:left; background-color: ' . $print->bg_color . '; color: ' . $print->text_color . ';"> ' . $print->membership_name . '</button>';
			$sub_array[] = ($print->membership_start != "" && $print->membership_end != "") ? date('d M Y',strtotime($print->membership_start)).' to '.date('d M Y',strtotime($print->membership_end)) : '-';;
			$sub_array[] = $print->membership_price;

			$sub_array[] = !empty($sold_by) ? $sold_by->full_name : ($print->is_counter == 'Yes' ? 'Counter Sale' : '-');
			if($print->buy_from == "1"){
				$sub_array[] = 'Mobile App';
			}else{
				$sub_array[] = 'Salon Panel';
			}
			if($print->payment_status == "1"){
				$sub_array[] = '<label class="label label-success">Completed</label>';
			}else{
				$sub_array[] = '<label class="label label-warning">Pending</label>';
			}

			if($print->membership_status == "0"){
				$sub_array[] = '<label class="label label-success">Active</label>';
			}elseif($print->membership_status == "1"){
				$sub_array[] = '<label class="label label-warning">Expired</label>';
			}elseif($print->membership_status == "2"){
				$sub_array[] = '<label class="label label-danger">Cancelled</label>On: ' . date('d-m-Y h:i A',strtotime($print->cancelled_on)); 
			}elseif(date('Y-m-d') < date('Y-m-d',strtotime($print->membership_end))){
				$sub_array[] = '<label class="label label-warning">Expired</label> '; 
			}else{
				$sub_array[] = '-';
			}
	...
	$TotalProducts = $this->Salon_model->get_membership_report_data_ajx_count($search);
...
public function get_package_report_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
	...
	$document = $this->Salon_model->get_package_report_data_ajx_list($length, $start, $search);
	...
	if(!empty($document)){
		...
		$offset = ($page - 1) * $length + 1;
		foreach($document as $print){
			$sold_by = $this->Salon_model->get_single_emp_details($print->allocated_by);

			$sub_array = array();
			$sub_array[] = $offset++;
			$sub_array[] = $print->full_name;
			$sub_array[] = $print->customer_phone;
			$sub_array[] = '<button class="btn btn-sm" style="float:left; background-color: ' . $print->bg_color . '; color: ' . $print->text_color . ';"> ' . $print->package_name . '</button>';
			$sub_array[] = ($print->package_start_date != "" && $print->package_start_date != "" && $print->package_end_date != "" && $print->package_end_date != "") ? date('d M, Y',strtotime($print->package_start_date)).' To '.date('d M, Y',strtotime($print->package_end_date)) : '-';
			$sub_array[] = $print->package_amount;
			if($print->is_booking_done == "1"){
				$sub_array[] = '<label class="label label-success">Completed</label>';
			}elseif($print->is_booking_done == '0'){
				$sub_array[] = '<label class="label label-warning">Pending</label>';
			}else{
				$sub_array[] = '-';
			}			
	...
	$TotalProducts = $this->Salon_model->get_package_report_data_ajx_count($search);
...
public function get_giftcard_report_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
	...
	$document = $this->Salon_model->get_giftcard_report_data_ajx_list($length, $start, $search);
	// echo '<pre>'; print_r($document);
	$data = array();
	if(!empty($document)){
		$page = $start / $length + 1;
		$offset = ($page - 1) * $length + 1;
		foreach($document as $asign_list_result){
			...
			if ($asign_list_result->gift_card_status == '0') {
				...
				$sub_array[] = '<label class="label label-warning">Used</label>';
			} else {
				$sub_array[] = '-';
			}
			$sub_array[] = $asign_list_result->full_name;
			$sub_array[] = $asign_list_result->customer_phone;
			$sub_array[] = '<button class="btn btn-sm" style="float:left; background-color: ' . $asign_list_result->bg_color . '; color: ' . $asign_list_result->text_color . ';"> ' . $asign_list_result->gift_name . '</button>';
			$sub_array[] = $asign_list_result->giftcard_customer_uid;
			$sub_array[] = $asign_list_result->gender == '0' ? 'Male' : ($asign_list_result->gender == '1' ? 'Female' : '-');

			$sub_array[] = $asign_list_result->gift_card_price;
			$sub_array[] = $asign_list_result->gift_card_balance;
			$sub_array[] = date('d-m-y h:i A', strtotime($asign_list_result->created_on));
			$sub_array[] = !empty($sold_by) ? $sold_by->full_name : ($asign_list_result->is_counter == 'Yes' ? 'Counter Sale' : '-');
	...
	$TotalProducts = $this->Salon_model->get_giftcard_report_data_ajx_count($search);
...
public function get_marketing_birthday_data_ajx(){
	...
	$document = $this->Marketing_model->get_marketing_data_ajx_list($length, $start, $search);
	$data = array();
	if(!empty($document)){
		$page = $start / $length + 1;
		$offset = ($page - 1) * $length + 1;
	...
	$TotalProducts = $this->Marketing_model->get_marketing_data_ajx_count($search);
...
public function get_trying_booking_marketing_report_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
	...
	if(!empty($document)){
		...
		foreach($document as $print){
			...
			$booking_status = '';
			if($print->booking_status == '0'){
				$booking_status = '<label class="label label-warning">Trying</label>';
			}elseif($print->booking_status == '1'){
				$booking_status = '<label class="label label-success">Booking Placed</label>';
				$booking_status .= '<a href="'.base_url().'booking-list?status=&id='.$print->trying_booking_confirm_id.'&customer='.$print->customer_id.'&from_date=&to_date=&service=&stylist=" style="text-decoration:underline;">View Booking</a>';
			}
...
public function get_socket_logs_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
	...
	$document = $this->Marketing_model->get_socket_logs_data_ajx($length, $start, $search);
	$data = array();
	if(!empty($document)){
		$page = $start / $length + 1;
		$offset = ($page - 1) * $length + 1;
		foreach($document as $print){
			$sub_array = array();
			if($print->message_type != ''){
				$message_type_raw = $print->message_type ?? '';
				$message_type = ucwords(str_replace('_', ' ', $message_type_raw));
			}else{
				$json_decode_data = $print->data != "" ? json_decode($print->data) : [];
				$message_type_raw = $json_decode_data->message_type ?? '';
				$message_type = ucwords(str_replace('_', ' ', $message_type_raw));
			}
			$sub_array[] = $offset++;
			$sub_array[] = $message_type;
			$sub_array[] = $print->created_on != "" ? date('d-m-Y h:i A',strtotime($print->created_on)) : '-';
			$sub_array[] = $print->message != "" ? $print->message : '-';
			$sub_array[] = $print->data != "" ? $print->data : '-';
			$sub_array[] = $print->error != "" ? $print->error : '-';
			$sub_array[] = $print->url != "" ? $print->url : '-';

			$data[] = $sub_array; 
		}
	}
	$TotalProducts = $this->Marketing_model->get_socket_logs_data_ajx_count($search);
...
public function get_marketing_booking_promotion_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
...
public function get_customize_message_remark_ajx() {
	$this->Marketing_model->get_customize_message_remark_ajx(); 
}
public function get_send_customize_message_form_ajx() {
	$this->Marketing_model->get_send_customize_message_form_ajx(); 
}
public function get_all_customize_messages_ajx(){
	...
	$document = $this->Marketing_model->get_all_customize_messages_ajx($length, $start, $search);
	$data = array();
	if(!empty($document)){
		$page = $start / $length + 1;
		$offset = ($page - 1) * $length + 1;
	...
	$TotalProducts = $this->Marketing_model->get_all_customize_messages_ajx_count($search);
...
public function get_employee_report_data_ajx(){
	...
	if($employee_report_type == '3' || $employee_report_type == '2' || $employee_report_type == '5'){
		...
		$selected_month = $this->input->post('selected_month') != "" ? $this->input->post('selected_month') : date('m');
		$selected_year = $this->input->post('selected_year') != "" ? $this->input->post('selected_year') : date('Y');
		$start_date = date("$selected_year-$selected_month-01");
		$end_date = date("Y-m-t", strtotime($start_date));
		$from_date = date("Y-m-d", strtotime($start_date));
		$to_date = date("Y-m-d", strtotime($end_date));
	}

    if($employee_report_type == '5'){
		$document = $this->Salon_model->get_employee_short_break_ajx_list($length, $start, $search, $from_date, $to_date);
	}else{
	...
	if(!empty($document)){
		...
		foreach($document as $print){
			...
			if($employee_report_type == '1'){
				...
				$emp_attendance = $this->Salon_model->get_employee_single_attendance_report_datewise($print->id,$from_date,$to_date);
			...
			}else if($employee_report_type == '5'){
				if($print->break_status == '0'){
					$break_status = '<label class="label label-success" style="color: black;">Active</label>';
				}elseif($print->break_status == '1'){
					$break_status = '<label class="label label-warning" style="color: black;">Removed</label>' . '' . ($print->cancelled_on != "" ? 'On: ' . date('d-m-Y h:i A',strtotime($print->cancelled_on)) : '');
				}elseif($print->break_status == '2'){
						$break_status = '<label class="label label-info" style="color: black;">Lapsed</label>';
				}else{
					$break_status = '-';
				}
				$sub_array[] = date('d-m-Y',strtotime($print->break_date));
				$sub_array[] = date('h:i A',strtotime($print->from)) . ' - ' . date('h:i A',strtotime($print->to));
				$sub_array[] = date('d-m-Y h:i A',strtotime($print->created_on));
				$sub_array[] = $break_status;
			}
			$data[] = $sub_array; 
		}
	}
    ...
    if($employee_report_type == '5'){
		$TotalProducts = $this->Salon_model->get_employee_short_break_ajx_count($search, $from_date, $to_date);
	}else{
...
public function get_whatsapp_report_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
	...
	$whatsapp_report_type = $this->input->post('whatsapp_report_type');
	if($whatsapp_report_type == '1'){
		$document = $this->Salon_model->get_employee_whatsapp_coins_report_list($length, $start, $search);
	...
	if(!empty($document)){
		...
		foreach($document as $print){
			...
			if($whatsapp_report_type == '1'){
				...
				$sub_array[] = '<button style="float:left;background:transparent !important;outline:none; box-shadow:none;" title="View Message" type="button" class="btn btn-primary event-action-button" id="service_details_button_'.$print->id.'" onclick="showMessage('.$print->id.')" data-toggle="modal" data-target="#messageModal"><i style="color:gray;font-size: 20px;margin-left: -5px;" class="fa-solid fa-comment"></i></button>';
				$sub_array[] = date('d M, Y h:i A',strtotime($print->created_on));
				$sub_array[] = $print->wp_coins_opening_balance != "" ? $print->wp_coins_opening_balance : '0';
				$sub_array[] = $print->wp_coins_used != "" ? $print->wp_coins_used : '0';
				$sub_array[] = $print->wp_coins_closing_balance != "" ? $print->wp_coins_closing_balance : '0';
...
public function get_trying_booking_report_data_ajx(){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$order = $this->input->post("order");
	...
	$document = $this->Salon_model->get_trying_booking_report_data_ajx($length, $start, $search);
	...
	if(!empty($document)){
		...
		foreach($document as $print){
			...
			if($print->step == '0'){
				...
				$step = 'Special Services Selection';
			}elseif($print->step == '2'){
				$step = 'Stylist & Timeslot Selection';
			}elseif($print->step == '3'){
				$step = 'Payment & Discounts Page';
			}
			...
			if($print->booking_status == '0'){
				...
				$booking_status = '<label class="label label-success">Booking Placed</label>';
				$booking_status .= '<a href="'.base_url().'booking-list?status=&id='.$print->booking_id.'&customer='.$print->customer_id.'&from_date=&to_date=&service=&stylist=" style="text-decoration:underline;">View Booking</a>';
			}

			$sub_array[] = $offset++;
			$sub_array[] = $print->customer_name . ' (' . $print->customer_phone . ')';
			$sub_array[] = $step != "" ? $step : '-';
			$sub_array[] = $print->service_names != "" ? $print->service_names : '-';
	...
	$TotalProducts = $this->Salon_model->get_trying_booking_report_data_ajx_count($length, $start, $search);
...
public function set_short_break_lapsed_data_ajx(){
	$this->Salon_model->set_short_break_lapsed_data_ajx();
} 
...
public function get_product_purchase_ajx(){
	$this->Salon_model->get_product_purchase_ajx();
}
public function get_product_purchase_rows_ajx(){
	$this->Salon_model->get_product_purchase_rows_ajx();
}
public function get_late_hr_div_ajx(){
	$this->Salon_model->get_late_hr_div_ajx();
}
public function get_salon_services_ajx(){
	$this->Salon_model->get_salon_services_ajx();
}
public function get_salon_products_ajx(){
	$this->Salon_model->get_salon_products_ajx();
}
public function setDashboardCalendar(){
	$this->Salon_model->setDashboardCalendar();
}
public function fetch_resources_ajx(){
	$result = $this->Calendar_model->fetch_resources();
	echo json_encode($result);
}
public function fetch_resources_events(){
	$this->Calendar_model->fetch_resources_events();
}
public function get_working_hours(){
	$this->Calendar_model->get_working_hours();
}
public function lock_time_period(){
	$this->Calendar_model->lock_time_period();
}
public function remove_short_break(){
	$this->Calendar_model->remove_short_break();
}
public function fetch_guest_count_ajax(){
	$this->Salon_model->fetch_guest_count_ajax();
}
public function fetch_appointment_ajax(){
	$this->Calendar_model->fetch_appointment_ajax();
}
public function reschedule_appointment_ajax(){
	$this->Calendar_model->reschedule_appointment_ajax();
}
}  
