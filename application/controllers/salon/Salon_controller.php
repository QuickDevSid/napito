	...
	public function add_new_booking_new(){
		...
		if ($this->input->post('customer_button') == 'customer_button') {
			...
			if($result == 'not_allowed'){
				$this->session->set_flashdata('message', 'Slot not available now, Please try again with another slot');
				redirect('add-new-booking-new?customer=' . $this->input->post('customer_name'));
			}else{
				if ($result != "") {
...
// add work shedule
	...
	public function trying_booking_list(){
		$this->load->view('salon/trying_booking_list');
	}
