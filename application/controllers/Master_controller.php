...
class Master_controller extends CI_Controller {
	...
	public function salon_marketing(){
		if($this->input->post('service_remindner_button') == 'service_remindner_button'){
			...
			$this->session->set_flashdata('success','Record updated successfully');
			redirect('salon-marketing');
		}elseif($this->input->post('emergency_bill_count_button') == 'emergency_bill_count_button'){
			$result = $this->Master_model->set_emergency_bill_count();
			$this->session->set_flashdata('success','Record updated successfully');
			redirect('salon-marketing');
	...
	public function add_help_section(){
		$this->form_validation->set_rules('help_title', 'help title', 'required');
		if ($this->form_validation->run() === FALSE) {
			$data['single'] = $this->Master_model->get_help_section();
			$this->load->view('admin/add_help_section', $data);
		} else {
			$result = $this->Master_model->set_help_section();
			if ($result == "0") {
				$this->session->set_flashdata('success', 'Record inserted successfully');
			} else {
				$this->session->set_flashdata('success', 'Record updated successfully');
			}
			redirect('app-help-management');
		}
	}
}	